...
<H1 class="title">Edit Tutor</H1>
	<div class="container">
		<div class="row landing">
			<div class="col-md-6 col-md-offset-3">
				<c:choose>
				    <c:when test='${message.equals("null")}'>
				    		<div class='alert alert-danger'><span class=''>${message}</span> </div>
				    </c:when>
			   	</c:choose>	
				<form method="post"action="<c:url value="/edittutor/${tutor.getId()}"/>">
					<div class="form-group">
						<label for="Staffid">Staff Identity</label> <input
							type="text" class="form-control" name="Staffid" id= "Staffid" value = "${tutor.getStaffID()}"></input>
					</div>
					<div class="form-group">
						<label for="Staffname">Staff Name</label> <input
							type="text" class="form-control" name="Staffname" id="Staffname" value = "${tutor.getName()}"></input>
					</div>
					<div class="form-group">
						<label for="Staffdept">Department</label> <input
							type="text" class="form-control" name="Staffdept" id="Staffdept" value = "${tutor.getDepartment()}"></input>
					</div>
