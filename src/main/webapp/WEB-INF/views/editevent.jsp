		...
		<div class="col-md-4 col-md-offset-1">
			<c:choose>
			   ...
			   <c:choose>
				...
				<form action="<c:url value="/editevent/${event.getId()}"/>" method="post">
					<div class="form-group">
						<label for="Studentgroup">Date</label>
							<div class="input-group">
      							...
      							<input class="form-control" id="date" required= "required" name="date" placeholder="DD/MM/YYYY" type="date" value="${newdate}"></input>
     						</div>
					</div>
					<div class="form-group">
						<label for="start">Start Time</label>
						<select class="selectpicker form-control" data-live-search="true" name="start" required= "required">
						<option value="${event.getStartTime()}" selected >${event.getStartTime()}</option>
					...
					<div class="form-group">
						...
						<option value="${event.getEndTime()}" selected >${event.getEndTime()}</option>
						  <option data-tokens="10:00" value="10:00">10:00</option>
						  <option data-tokens="11:00" value="11:00">11:00</option>
						  <option data-tokens="12:00" value="12:00">12:00</option>
						  <option data-tokens="13:00" value="13:00">13:00</option>
					 ...
					 <div class="form-group">
						...
						<option value="${event.getRoom().getId()}" selected>${event.getRoom().getRoomNumber()}</option>
					...
					<div class="form-group">
						...
						<option value="${event.getModule().getId()}" selected >${event.getModule().getModuleCode()}</option>
					...
					<div class="form-group">
						...
						<c:forEach  items="${student}" var="studentValue">
								<c:forEach  items="${eventstudent}" var="eventstudentValue">
								
								<c:choose>
				    				<c:when test="${eventstudentValue.getGroupId()==studentValue.getGroupId()}">
				    					<option data-tokens="${studentValue.getGroupName()}" value="${studentValue.getGroupId()}" selected>${studentValue.getGroupName()}</option>
				    				</c:when>
			   					</c:choose>
									<c:choose>
				    				<c:when test="${eventstudentValue.getGroupId()!=studentValue.getGroupId()}">
					...
					<div class="form-group">
						...
						<select class="selectpicker form-control" data-live-search="true" name="tutor" multiple>
						      ...
						      <c:forEach  items="${tutor}" var="tutorValue">
								<c:forEach  items="${eventtutor}" var="eventtutorValue">
								
									<c:choose>
					    				<c:when test="${eventtutorValue.getId()==tutorValue.getId()}">
					    					<option data-tokens="${tutorValue.getName()}" value="${tutorValue.getId()}" selected>${tutorValue.getName()}</option>
					    				</c:when>
				   					</c:choose>
										<c:choose>
					    				<c:when test="${eventtutorValue.getId()!=tutorValue.getId()}">
