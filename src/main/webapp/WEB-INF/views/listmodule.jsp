					...
					<c:if test="${module.size() > 0}">
						<c:forEach  items="${module}" var="moduleValue">
							<tr>
								...
								<td class=""><a href="<c:url value="/editmodule/${moduleValue.getId()}"/>"><button class="btn btn-success">edit</button></a></td>
								<td class=""><a href="<c:url value="/deletemodule/${moduleValue.getId()}"/>"><button class="btn btn-danger">delete</button></a></td>
							</tr>
						</c:forEach>
					</c:if>
				</table>
